...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
	...
	// V>>();
	protected final BalanceableBinaryTree<K, V> tree = new BalanceableBinaryTree<>();
	...
	public V put(K key, V value) throws IllegalArgumentException {
		...
		if(isExternal(p)) {
			...
			V changed = p.getElement().getValue();
			set(p, newest);
			rebalanceAccess(p);
			return changed;
		}
	...
	public Iterable<Entry<K, V>> subMap(K fromKey, K toKey) throws IllegalArgumentException {
		...
		if(compare(fromKey, toKey) < 0) {
			mapRecursive(fromKey, toKey, root(), buffer);
		}
		
		return buffer;
	}
	
	private void mapRecursive(K fromKey, K toKey, Position<Entry<K, V>> p, ArrayList<Entry<K, V>> buffer) {
        if (isInternal(p)) {
        	
            if (compare(p.getElement(), fromKey) < 0) {
            	mapRecursive(fromKey, toKey, right(p), buffer);
            } else {
            	mapRecursive(fromKey, toKey, left(p), buffer);
